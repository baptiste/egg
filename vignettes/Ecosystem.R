...
library(gtable)
library(grid)
opts_chunk$set(message=FALSE, fig.width=6, fig.height=3, cache=FALSE)
...
layouts <- arrangeGrob(grobs=pl, widths=c(1.2,1,1),
                       layout_matrix = rbind(c(1, 2, 3),
                                             c(4, 4, 4)))
...
legend <- gtable_matrix("legend", matrix(leg, ncol=1), 
                        widths=1.2*grobWidth(leg[[1]]),
                        heights=unit(rep(1,length(leg)),"line"))
legend <- gtable_add_cols(legend, unit(1,"line"), 0)

legend <- gtable_add_grob(legend, t=seq_along(leg), l=1,
                          lapply(cols[seq_along(ids)], function(x) 
                            rectGrob(gp=gpar(fill=x, col=NA))))
grid.arrange(layouts, legend, widths = unit.c(unit(1,"null"),
...
| wheel2            | `ground_braking_rotating_object` | ? | ? |
"
cat(tabl) 

...
gtable_frame <- function(g, width=unit(1, "null"), height=unit(1, "null"), debug=FALSE) {
  ...
  all <- gtable::gtable_matrix(
    "all", grobs = matrix(grobs, ncol = 3, nrow = 3, byrow = TRUE),
  ...
  if (debug) {
    ...
    all <- gtable::gtable_add_grob(
      all, replicate(9, hints, simplify = FALSE),
...
.tmp <- gtable::gtable_matrix(
  "placeholder", matrix(replicate(9, grid::nullGrob(), simplify = FALSE), 3, 3),
...
ggarrange <- function(..., plots = list(...),
  ...
  fg <- mapply(
    gtable_frame, g = grobs, width = widths, height = heights,
  ...
  if (is.grob(top)) {
    ...
    gt <- gtable_add_grob(
      gt, top, t = 1, l = 1, r = ncol(gt), z = Inf,
      clip = clip
    )
  }
